...
fn check_auth(state: &ActionState, headers: &HeaderMap) -> Result<(), Response> {
...
mod tests {
    ...
    async fn spawn_sched() -> (SchedulerHandle, Arc<MemoryStore>, tokio::task::JoinHandle<()>) {
        ...
        crate::store::PendingStore::save(&*store, &map).await.unwrap();

        let sched = Scheduler::new(store.clone(), notifier).await.unwrap();
        sched.set_clock(Utc.with_ymd_and_hms(2026, 5, 1, 8, 0, 0).unwrap()).await;
