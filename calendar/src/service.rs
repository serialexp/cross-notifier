...
pub async fn dry_run(source: &dyn CalendarSource, horizon: Duration) -> anyhow::Result<Vec<crate::types::Occurrence>> {
    let ics = source.fetch().await?;
...
async fn run_summary_once(
    ...
    let tomorrow_local = now_local.date_naive().succ_opt().ok_or_else(|| {
        anyhow::anyhow!("chrono refused to advance today's date — not expected")
    })?;
...
mod tests {
    ...
    fn next_fire_is_today_when_noon_still_ahead() {
        ...
        let now = Local
            ...
            .earliest()
            .unwrap();
        let cfg = DailySummaryConfig { hour: 12, minute: 0 };
