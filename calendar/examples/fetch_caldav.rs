...
async fn main() -> anyhow::Result<()> {
    ...
    let source: Arc<dyn CalendarSource> = if let Ok(path) = env::var("CAL_ICS_FILE") {
        ...
        let endpoint = env::var("CALDAV_ENDPOINT")
            .map_err(|_| anyhow::anyhow!("CALDAV_ENDPOINT not set (and no CAL_ICS_FILE fallback)"))?;
        let user = env::var("CALDAV_USER")
            .map_err(|_| anyhow::anyhow!("CALDAV_USER not set"))?;
        let password = env::var("CALDAV_PASSWORD")
            .map_err(|_| anyhow::anyhow!("CALDAV_PASSWORD not set"))?;
