...
EOF
    ...
    echo "    ${APP_NAME} -port 8080               # Start daemon on custom port"
    echo "    ${APP_NAME} -install-autostart        # Enable auto-start on login"
    echo "    ${APP_NAME} -uninstall-autostart      # Disable auto-start on login"
    echo ""
