...
import type { Notification, NotificationStatus, Action } from "./types.js";
...
async function main(): Promise<void> {
  ...
  if (!values.server) missing.push("--server");
  if (!values.token) missing.push("--token");
  if (!values.title) missing.push("--title");
  if (!values.message) missing.push("--message");
  ...
  const notification: Notification = {
    ...
    status: values.status as NotificationStatus | undefined,
