...
impl App {
    fn new(event_loop: &EventLoop<AppEvent>, port: u16) -> Self {
        ...
        let store: SharedStore = Arc::new(std::sync::RwLock::new(
        ...
        let watcher_result = notify::recommended_watcher(move |res: Result<notify::Event, notify::Error>| {
            if let Ok(event) = res {
                ...
                let is_config = event.paths.iter().any(|p| {
                    p.file_name().map(|f| f.to_os_string()) == config_filename
                });
                if is_config {
    ...
    fn update_tray_icon(&self) {
        if let Some(tray) = &self.tray {
            ...
            let has_notifications = !self.queue.is_empty() || store_count > 0;
            tray.set_has_notifications(has_notifications);
    ...
    fn handle_notification(
        ...
        // Rule matching
        let (sound_name, action) = match rules::match_rule(&payload, &server_label, &self.config.rules) {
        ...
        if action == config::RuleAction::Dismiss {
            info!("Rule dismissed notification: {} - {}", payload.title, payload.message);
            return;
        }

        // Silent: store to center only, no popup or sound
        if action == config::RuleAction::Silent {
            info!("Rule silenced notification: {} - {}", payload.title, payload.message);
    ...
    fn execute_action(&mut self, store_id: i64, action_idx: usize) {
        ...
        info!("Executing action '{}' on notification {}", action.label, store_id);
        ...
        if action.open {
            ...
            self._runtime.spawn(async move {
                ...
                match req.send().await {
                    ...
                    Err(e) => {
                        tracing::warn!("Action request failed for notification {}: {}", store_id, e);
                    }
                }
            });
        }
    ...
    fn handle_center_window_event(&mut self, _event_loop: &ActiveEventLoop, event: WindowEvent) {
        match event {
            ...
            WindowEvent::Focused(false) => {
                if self.center.as_ref().is_some_and(|c| c.should_close_on_focus_loss()) {
                    self.close_center();
                }
            }
            _ => {}
    ...
    fn render_popup(&mut self) {
        ...
        if !expired.is_empty() {
            ...
            if let Some(tray) = &self.tray {
                tray.set_has_notifications(!self.queue.is_empty());
            }
        }
        // Store expired notifications in center
        for expired_notif in &expired {
            if expired_notif.payload.store_on_expire {
                self.store
                    .write()
                    .unwrap()
                    .add(expired_notif.payload.clone(), expired_notif.server_label.clone());
            }
        }
        ...
        let notification_refs: Vec<&notification::Notification> =
            self.queue.visible().iter().take(card::MAX_VISIBLE).collect();
...
impl ApplicationHandler<AppEvent> for App {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let (gpu, popup_surface) = pollster::block_on(GpuContext::new(window.clone()))
        ...
        info!(
            "Renderer initialized, listening on port {}",
            self.port
        );
    }
        ...
        match event {
            ...
            AppEvent::NotificationResolved(resolved) => {
                ...
                if let Some(local_id) =
                    self.queue.find_by_server_id(&resolved.notification_id)
                {
                    let had_more = self.queue.visible().len() > 1;
...
fn main() -> anyhow::Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-help" | "--help" | "-h" => {
                println!("Usage: cross-notifier [-port N] [-install-autostart] [-uninstall-autostart]");
                return Ok(());
            }
            arg => {
                eprintln!("Unknown flag: {}", arg);
                eprintln!("Usage: cross-notifier [-port N] [-install-autostart] [-uninstall-autostart]");
                std::process::exit(1);
            }
        }
        i += 1;
