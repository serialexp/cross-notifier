...
pub fn router(state: CoreState) -> Router {
    Router::new()
        ...
        .route(
            "/devices",
            get(handle_list_devices).post(handle_register_device),
        )
...
async fn handle_register_device(
    ...
    let device = registry
        .register(req.device_token, req.label, req.platform)
        .await;
    info!(
...
async fn handle_list_devices(State(state): State<CoreState>, headers: HeaderMap) -> Response {
    if !check_auth(&headers, state.secret()) {
        return unauthorized();
    }
    let Some(registry) = state.devices() else {
            ...
            incoming = stream.next() => {
                match incoming {
                    ...
                    Some(Ok(WsMessage::Ping(data))) => {
                        let send_result = sink.send(WsMessage::Pong(data)).await;
...
mod push_integration_tests {
    ...
    async fn notify_fans_out_to_registered_devices() {
        ...
        let tokens: std::collections::HashSet<String> =
            h.mock.requests().into_iter().map(|r| r.token).collect();
