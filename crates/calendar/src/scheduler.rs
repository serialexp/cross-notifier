...
impl Scheduler {
    ...
    async fn on_tick(&mut self) {
        ...
        self.pending.retain(|_, p| {
            p.fired_at.map(|t| t > cutoff).unwrap_or(true) || p.snoozed_until.is_some()
        });
...
mod tests {
    ...
    async fn refresh_inserts_occurrences() {
        ...
        s.set_clock(Utc.with_ymd_and_hms(2026, 5, 1, 9, 0, 0).unwrap())
            .await;
        s.on_refresh(vec![occ(
            "a",
    ...
    async fn snooze_rearms_fire() {
        ...
        let t0 = Utc.with_ymd_and_hms(2026, 5, 1, 10, 0, 0).unwrap();
        s.set_clock(t0).await;
        s.on_refresh(vec![occ("x", t0 - Duration::minutes(5))])
            .await;
    ...
    async fn dismiss_prevents_refire() {
        ...
        let t0 = Utc.with_ymd_and_hms(2026, 5, 1, 10, 0, 0).unwrap();
        s.set_clock(t0).await;
        s.on_refresh(vec![occ("x", t0 - Duration::minutes(1))])
            .await;
        s.on_tick().await;
    ...
    async fn refresh_drops_ghost_occurrences() {
        ...
        let t0 = Utc.with_ymd_and_hms(2026, 5, 1, 10, 0, 0).unwrap();
        s.set_clock(t0).await;
        s.on_refresh(vec![occ("ghost", t0 + Duration::hours(1))])
            .await;
        assert_eq!(s.pending_count(), 1);
