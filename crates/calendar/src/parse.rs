...
use icalendar::parser::{Component as RawComponent, read_calendar, unfold};
use rrule::{RRuleSet, Tz as RTz};
use tracing::{debug, warn};

use crate::types::{EventInstance, Occurrence};

pub fn parse(ics: &str, now: DateTime<Utc>, horizon: Duration) -> anyhow::Result<Vec<Occurrence>> {
    ...
    if parse_errors > 0 {
        warn!(
            count = parse_errors,
            "calendar: total parse errors this refresh"
        );
    }
...
///   so the last day's instances are still included.
