...
use cross_notifier_calendar::{
    KIND_CALDAV, KIND_ICS_FILE, KIND_ICS_URL, fingerprint as calendar_fingerprint, strip_auth,
};
...
impl SettingsState {
    fn from_config(config: &Config, connection_status: &HashMap<String, ConnectionState>) -> Self {
        ...
        let rules: Vec<NotificationRule> = self
            ...
            .map(|r| {
                ...
                let status = if r.status_idx == 0 {
                    ...
                    STATUS_OPTIONS.get(r.status_idx).unwrap_or(&"").to_string()
                };
    ...
    pub fn render(&mut self, gpu: &GpuContext) {
        ...
        let output = match self.surface.surface.get_current_texture() {
            ...
            Err(wgpu::SurfaceError::Outdated | wgpu::SurfaceError::Lost) => {
                self.surface
                    .resize(gpu, self.surface.size.0, self.surface.size.1);
                return;
            }
            Err(e) => {
    ...
    for i in 0..state.servers.len() {
        ui.vertical(|ui| {
            ...
            if !state.servers[i].connected
                && let Some(err) = &state.servers[i].last_error
            {
                ui.horizontal(|ui| {
            ...
            .show(ui, |ui| {
                    ...
                    egui::ComboBox::from_id_salt(format!("rule_server_{}", i))
                        ...
                        .show_ui(ui, |ui| {
                            for (idx, opt) in server_options.iter().enumerate() {
                                ui.selectable_value(&mut state.rules[i].server_idx, idx, opt);
                            }
                        });
                    ...
                    egui::ComboBox::from_id_salt(format!("rule_status_{}", i))
                        ...
                        .show_ui(ui, |ui| {
                            for (idx, opt) in STATUS_OPTIONS.iter().enumerate() {
                                ui.selectable_value(&mut state.rules[i].status_idx, idx, *opt);
                            }
                        });
                ...
                ui.horizontal(|ui| {
                    ...
                    egui::ComboBox::from_id_salt(format!("rule_action_{}", i))
                        ...
                        .show_ui(ui, |ui| {
                            for (idx, opt) in ACTION_OPTIONS.iter().enumerate() {
                                ui.selectable_value(&mut state.rules[i].action_idx, idx, *opt);
                            }
                        });
                    ...
                    if state.rules[i].action_idx == 0 {
                        ...
                        if has_sound && ui.button("\u{25B6}").clicked() {
...
fn collect_server_feeds(
    ...
    let mut out = Vec::new();
    for s in servers {
        let Some(st) = connection_status.get(&s.url) else {
            continue;
        };
        let server_label = if s.label.is_empty() {
            s.url.clone()
        } else {
            s.label.clone()
        };
...
fn draw_calendar(ui: &mut egui::Ui, state: &mut SettingsState) {
    ...
    ui.horizontal(|ui| {
        ui.label("Look ahead:");
        ui.add(
            egui::DragValue::new(&mut cal.horizon_hours)
                .range(1..=720)
                .speed(1.0),
        );
        ui.label("h");
        ...
        ui.add(
            egui::DragValue::new(&mut cal.refresh_minutes)
                .range(1..=240)
                .speed(1.0),
        );
        ui.label("min");
        ...
        ui.add(
            egui::DragValue::new(&mut cal.snooze_hours)
                .range(1..=24)
                .speed(1.0),
        );
        ui.label("h");
...
fn apply_autostart_from_state(state: &SettingsState) {
    ...
    } else if !state.autostart_enabled
        && currently_installed
