...
use cross_notifier_calendar::{
    CalDav, CalendarHandleSlot, CalendarService, CalendarServiceConfig,
    CalendarSource as CalSource, CoreNotifier, CoreNotifierConfig, DailySummaryConfig, IcsFile,
    IcsUrl, JsonStore, calendar_action_router,
};
use cross_notifier_core::{CoreState, OutboundMessage, Subscriber, router as core_router};
...
pub async fn run_server(
    ...
    // on the same machine.
    let app = core_router(core).merge(daemon_routes).nest(
        "/calendar/action",
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            let ev = match msg {
                ...
                OutboundMessage::Resolved(r) => AppEvent::NotificationResolved(ResolvedMessage {
                ...
                OutboundMessage::Expired(e) => AppEvent::NotificationExpired(ExpiredMessage {
    ...
    let source: Arc<dyn CalSource> = match &cfg.source {
        crate::config::CalendarSource::CalDav {
            ...
            if endpoint.is_empty() || user.is_empty() || password.is_empty() {
                tracing::warn!("calendar: CalDAV source has blank fields; not starting",);
                return None;
            }
            Arc::new(CalDav::new(
                endpoint.clone(),
...
async fn handle_status(State(state): State<AppState>) -> Json<HashMap<String, bool>> {
...
async fn handle_center_list(
    State(state): State<AppState>,
) -> Json<Vec<CenterNotificationResponse>> {
...
async fn handle_center_dismiss(State(state): State<AppState>, Path(id): Path<i64>) -> StatusCode {
